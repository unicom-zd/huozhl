...
def dt_run(train_dir,test_dir,flag,tree_type,samsplit,samleaf,weight0,weight1):
        count=0
        print "decision tree:"
        ...
        clf = tree.DecisionTreeClassifier(criterion=tree_type,min_samples_split=samsplit,min_samples_leaf=samleaf,class_weight={0:weight0,1:weight1})
        print(clf)
        clf.fit(x_train, y_train)

        
        '''
        feature_names=[str(i) for i in xrange(87)]
        target_names='lost_tag'
...
if (platform.system()=='Windows'):
        dt_run('ZDYCL_01_11_PUB_8_2.csv','ZDYCL_02_12_PUB_8_2.csv',0,'entropy',100,50,0.52,1)
elif (platform.system()=='Linux'):
        try:
                dt_run(sys.argv[1],sys.argv[2],int(sys.argv[3]),sys.argv[4],int(sys.argv[5]),int(sys.argv[6]),float(sys.argv[7]),float(sys.argv[8]))
